...
const api = {
  ...
  setToken(t) { this.token = t; if (t) localStorage.setItem('techilla_token', t); else localStorage.removeItem('techilla_token'); },
  ...
  getMyProfile: (page = 0, size = 1) => api.get(`/profile/me?page=${page}&size=${size}`),
...
const auth = {
  getUser() { try { return JSON.parse(localStorage.getItem('techilla_user') || 'null'); } catch { return null; } },
  setUser(u) { if (u) localStorage.setItem('techilla_user', JSON.stringify(u)); else localStorage.removeItem('techilla_user'); },
  isLoggedIn() { return !!this.getUser() && !!api.token; },
  logout() { this.setUser(null); api.setToken(null); localStorage.removeItem('techilla_profile'); }
};
